...
Theme object contains keys for `screens`, `colors`, `space` and every plugins available.

See the [theme configuration reference](@TODO-link) or the [default theme](@TODO-link) for a complete list of theme options.
...
To learn more, see the [breakpoint customization documentation](@TODO-link).
...
These values are used for every plugins: `backgroundColor`, `borderColor`, `textColor`, and others...

To learn more, see the [color customization documentation](@TODO-link).
...
These values are used for every space relative plugins: `padding`, `margin`, `gap`, and others...

To learn more, see the [spacing customization documentation](@TODO-link).

## Core Plugins

Each plugin has its own section. For example `radii` key lets you customize which border radius utilities will be generated:
...
For a complete reference of available theme properties and their default values, see [the default theme configuration](@TODO-link).
...
For example if you create a custom filter plugin, or if you use it by your own, you can add a `filters` section:
