...
The v2 of xstyled favoured the usage of the `x` instead of the `styled` approach. Even if the `x` approach is always supported, it is better to use `x` as much as possible!
...
> If you have not customized `space` in your theme, specify old `space` in your theme:
...
> If you were not using any theme, read [customize theme documentation](/docs/theme/) to learn how to create a theme.
...
> If you use utility groups like `backgrounds`, `basics`, `borders`, `flexboxes`, `grids`, `layout`, `positioning`, `shadows`, `space`, `svg`, `typography` or `xgrids`. Then you have to pick new groups of utilities or single utilities. Read [how to create utilities](/docs/adding-new-utilities/) to learn more about it.
...
> If you use breakpoints and does not define them, add previous ones:
...
> If you want to add back `variant` utility:
