...
<>
  <template preview>
    <x.div display="flex" justifyContent="space-around">
      <x.div as="span" display="inline-flex" borderRadius="md" boxShadow="sm">
        <x.div
          as="button"
          type="button"
          display="inline-flex"
...
<x.button type="button" bg="indigo-600" disabled>
  <x.svg motionSafeAnimation="spin" h={5} w={5} mr={3} viewBox="0 0 24 24">
    {/* ... */}
  </x.svg>
  Processing
</x.button>
...
Using xstyled's `styled`, all animations defined are automatically binded to `animation` attribute:
