  ...
  <template preview>
    <x.div spaceY={4}>
      {['tighter', 'tight', 'normal', 'wide', 'wider', 'widest', '20px'].map(
        (letterSpacing) => (
          <x.dl key={letterSpacing} color="pink-600" overflow="hidden">
            <x.dt
              w={16}
              flexShrink={0}
              fontSize="sm"
...
Using xstyled's `styled`, all letter spacings defined are automatically binded to `letter-spacing` attribute:
