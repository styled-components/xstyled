  ...
  <template preview>
    <x.div spaceY={8}>
      {['none', 'tight', 'snug', 'normal', 'relaxed', 'loose'].map(
        (lineHeight) => (
          <x.dl key={lineHeight} color="light-blue-600" overflow="hidden">
            <x.dt
              w={16}
              flexShrink={0}
              fontSize="sm"
  ...
  <template preview>
    <x.div spaceY={8}>
      {[3, 4, 5, 6, 7, 8, 9, 10, '3rem', '20px'].map((lineHeight) => (
        <x.dl key={lineHeight} color="emerald-600" overflow="hidden">
          <x.dt
            w={16}
            flexShrink={0}
            fontSize="sm"
...
Using xstyled's `styled`, all line heights defined are automatically binded to `line-height` attribute:
