...
<>
  <template preview>
    <Box maxWidth="sm" mx="auto" p={6} display="flex" alignItems="center" bg="white" borderRadius="xl" boxShadow="md" spaceX={4}>
      <Box flexShrink={0}>
        <Box.svg h={12} w={12} viewBox="0 0 40 40">
          <defs>
      ...
      <Box>
        <Box fontSize="xl" fontWeight="medium" color="black">ChitChat</Box>
        <Box.p color="gray-500">You have a new message!</Box.p>
      </Box>
    </Box>
  </template>
...
- **Responsive design** — Unfortunately, media queries are not supported in inline styles. xstyled provides [responsive variants](@TODO-link) to build responsive interface.
- **Hover, focus, and other states** — Inline styles can't target states like hover or focus, but xstyled [state props](@TODO-link) make it easy to style those.
