---
menu: Guides
title: System Components
order: 0
---

# System Components

`@xstyled/styled-components` gives you system components out of the box.

## Use the Box component

The `Box` component has all system props built-in.

```js
import { Box } from '@xstyled/styled-components'

const PrimaryBox = props => <Box color="primary" m={2} {...props} />
```

## Create a system component

Additionally to all shorthand html tags available in `styled-components`, `@xstyled/styled-components` exposes a `box`.

```js
import styled from '@xstyled/styled-components'

/* Creates a `div` box */
const RedBox = styled.box`
  background-color: red;
`

<RedBox m={2} />
```

All tags also have their `xxBox` variants:

```js
import styled from '@xstyled/styled-components'

/* Creates a `a` box */
const ABox = styled.aBox`
  color: primary;
`

<ABox href="/about">Link</ABox>
```

## Use `forwardedAs` property

On all `Box` components, use `forwardedAs` property instead of `as` to avoid undesired attributes on the HTML element.

```js
import styled from '@xstyled/styled-components'

/* Creates a `div` box */
const RedBox = styled.box`
  background-color: red;
`

/* Replace `div` by `p`, but it is still a box! */
<RedBox forwardedAs="p" m={2} />
```

## Create your own box

If you don't want to use `@xstyled/styled-components`, you can create your own box component:

```js
import React from 'react'
import styled from 'styled-components'
import { system, createSystemComponent } from '@xstyled/system'

const InnerBox = createSystemComponent(React)
const Box = styled(InnerBox)(system)
```

> We use `createSystemComponent` to avoid undesired attributes on the HTML element.

See all available [system prop utilities](/docs/system-props/).
